...
SELECT 
    p.id,
    -- 24시간 이내면 (New), 댓글 3개 이상이면 (Best)를 제목 앞에 붙임
    CONCAT(
    ...
    p.created_at
FROM post p
-- 작성자 이름을 가져오기 위해 member 테이블을 연결
LEFT JOIN member m ON p.member_id = m.id
-- 댓글 수를 세기 위해 reply 테이블을 연결
LEFT JOIN reply r ON p.id = r.post_id
-- COUNT 같은 집계 함수가 들어갔으니 그룹화 진행
...
SELECT 
    ...
    p.content AS post_content,          -- 게시글 내용
    m.name AS post_writer_name,        -- 게시글 작성자 이름 (pm 테이블에서 가져옴)
    m.email AS post_writer_email,      -- 게시글 작성자 이메일 (pm 테이블에서 가져옴)
    rm.name AS reply_writer_name,       -- 댓글 작성자 이름 (rm 테이블에서 가져옴)
    r.content AS reply_content          -- 댓글 내용
FROM post p
-- 1. 게시글 작성자 정보를 가져오기 위해 member 테이블 연결
LEFT JOIN member m ON p.member_id = m.id
-- 2. 게시글에 달린 댓글들을 가져오기 위해 reply 테이블 연결
LEFT JOIN reply r ON p.id = r.post_id
-- 3. 댓글을 쓴 사람의 이름을 가져오기 위해 member 테이블을 "한 번 더" 연결
LEFT JOIN member rm ON r.member_id = rm.id;


-- 3. 게시글 등록 (새로운 게시글 작성)
INSERT INTO post (member_id, title, content, created_at, view_count) VALUES
    (3, '260616 문제 등록예시',' 오늘 작성한 새로운 게시글 작성 등록 예시 입니다!', '2026-06-16 16:20:00', 1);


-- 4. 게시글 수정 (2번 게시글의 제목과 내용 변경)
UPDATE post 
SET title = '260616 2번 게시글 제목 수정합니다', content = '2번 게시물 내용 수정도 합니다!'
WHERE id =2;



-- 5. 게시글 삭제 (3번에서 등록한 새로운 게시글삭제)
DELETE FROM post 
WHERE id = 21;
